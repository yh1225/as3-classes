...
package gs {
	...
	dynamic public class TweenGroup extends Proxy implements IEventDispatcher {
		public static const version:Number = 0.995;
			...
			if ($tweens.length != 0) {
				...
				for (i = tweens.length - 1; i > -1; i--) {
					...
					if (startTime > time) { //don't allow tweens with delays that haven't expired yet to be active
						if (!tween.active) { //otherwise a TweenGroup could be paused immediately after creation, then resumed later and if there's a sequential tween of an object, initTweenVals() could get called before the previous tweens finished running, meaning if the tweens pertain to the same property, the wrong value(s) could get recorded in the tweens Array
							render = false;
						}
						tween.active = false;
					}
